...
function homePage() {
    $(document).ready(function () {
        $.get('https://5d76bf96515d1a0014085cf9.mockapi.io/product', function (productData, status) {
            ...
            document.getElementById("banner-dynamic").style.display = "block";
            mainEle.innerHTML = '';
...
let productCall = (product) => {
    document.getElementById("banner-dynamic").style.display = "none";
    mainEle.innerHTML = '';
    ...
    for (let data in product) {
        ...
        if (data == 'name') {
            ...
            // btnEle.setAttribute('onclick', 'addCart()');
...
function cartPagePortal() {
    //
    $("#banner-dynamic").css({ display: "none" });
    let data = JSON.parse(localStorage.getItem("cartItems"));
    if (data == null) {
        data = [];
    }
